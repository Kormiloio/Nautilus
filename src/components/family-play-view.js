...
function renderSharedContent(step, turnPerson, quizState = null, participants = [], familyPeople = [], matchState = null) {
  ...
  if (step.type === 'family-story') {
    const cards = step.cards.map(card => `<article class="family-story-card"><span>Card ${card.number}</span><strong>${escapeHtml(card.prompt)}</strong><b>${renderLanguageRun(card.targetText, 'target', LANGUAGE_PACK, card)}</b><small>${renderLanguageRun(card.supportText, 'support', LANGUAGE_PACK, card)}</small><button class="dialogue-play-btn" data-family-audio="${card.number - 1}" aria-label="Play card ${card.number}">► Hear it</button></article>`).join('');
    return `${turnPrompt}<div class="family-activity-instructions"><strong>Three-card family story</strong><span>Read each card, put the story in order, and say the target sentence together. Add one silly detail before moving to the next card.</span><em>No recording—your family is the storyteller.</em></div><div class="family-story-grid">${cards}</div>`;
  }
  if (step.type === 'family-roleplay') {
    const turns = step.turns.map((turn, index) => `<article class="family-roleplay-turn"><span>${index + 1}</span><small>${escapeHtml(turn.role)}’s turn</small><strong>${renderLanguageRun(turn.targetText, 'target', LANGUAGE_PACK, turn)}</strong><em>${renderLanguageRun(turn.supportText, 'support', LANGUAGE_PACK, turn)}</em><button class="dialogue-play-btn" data-family-audio="${index}" aria-label="Play role ${index + 1}">► Hear it</button></article>`).join('');
    return `${turnPrompt}<div class="family-activity-instructions"><strong>Family role-play</strong><span>${escapeHtml(step.subtitle)}</span><em>Take turns, then swap roles. Nautilus does not record anyone.</em></div><div class="family-roleplay-grid">${turns}</div>`;
  }
  if (step.type === 'family-grammar') {
    const cards = step.items.map((item, index) => `<article class="family-grammar-card"><span>${index + 1}</span><strong>${renderLanguageRun(item.targetText, 'target', LANGUAGE_PACK, item)}</strong><small>${renderLanguageRun(item.supportText, 'support', LANGUAGE_PACK, item)}</small><button class="dialogue-play-btn" data-family-audio="${index}" aria-label="Play ${escapeHtml(item.targetText)}">► Hear it</button><em>Say it together, then switch roles</em></article>`).join('');
    return `${turnPrompt}<div class="family-activity-instructions"><strong>Family speaking game</strong><span>${escapeHtml(step.subtitle || 'Act it out, then say the answer together.')}</span><em>Nautilus does not record anyone. Your family does the speaking together.</em></div><div class="family-grammar-grid">${cards}</div>`;
  }
