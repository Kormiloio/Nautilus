...
describe('Offline Sync Queue', () => {
  ...
  it('should enqueue transaction to local queue when profile is a synced UUID', () => {
    ...
    const profiles = [{ id: 'db-profile-uuid', name: 'Mia', isGuide: false }];
  ...
  it('should successfully drain the transaction queue when online and authenticated', async () => {
    ...
    enqueueTransaction('db-profile-uuid', 'completeLesson', { name: 'Mia', lessonId: 'voyage-1' });
  ...
  it('should normalize legacy cloud profiles that were cached as guides', () => {
    localStorage.setItem('mn_profiles', JSON.stringify([
      { id: 'db-profile-uuid', name: 'Mia', isGuide: true },
    ]));
