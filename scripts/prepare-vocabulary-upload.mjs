...
const directory = resolve(process.argv[2] || '/private/tmp/nautilus-vocabulary-upload');
const release = execFileSync(process.execPath, ['scripts/build-pilot-release.mjs'], { encoding:'utf8', maxBuffer:16*1024*1024 });
const blocks = release.match(/do \$catalog\$ begin[\s\S]*?end \$catalog\$;/g) || [];
if (blocks.length !== 354) throw new Error('Expected exactly 354 pilot definitions');
mkdirSync(directory, { recursive:true });
const setup = `begin;
create table public.nautilus_catalog_stage_revision6 (like public.verified_lesson_catalog including all);
alter table public.nautilus_catalog_stage_revision6 enable row level security;
revoke all on public.nautilus_catalog_stage_revision6 from public,anon,authenticated,service_role;
commit;\n`;
writeFileSync(resolve(directory,'00-create-private-stage.sql'),setup);
for (let index=0; index<blocks.length; index+=3) {
  const sql = 'begin;\n'+blocks.slice(index,index+3).map(block=>block.replaceAll('public.verified_lesson_catalog','public.nautilus_catalog_stage_revision6')).join('\n')+'\ncommit;\n';
...
const insert = `
do $$ begin
  if (select count(*) from public.nautilus_catalog_stage_revision6)<>354 then raise exception 'Staging upload incomplete'; end if;
end $$;
...
select id,pack_id,pack_version,lesson_id,revision,mode,voyage_position,exercises,topic_id,is_checkpoint from public.nautilus_catalog_stage_revision6
...
publish=publish.replace('commit;\nselect public.get_verified_catalog_status()', 'drop table public.nautilus_catalog_stage_revision6;\ncommit;\nselect public.get_verified_catalog_status()');
...
console.log(`Prepared 120 SQL files in ${directory}; no database connection made.`);
