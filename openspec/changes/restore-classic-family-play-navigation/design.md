...
The repair removes only the incompatible segment-navigation restriction for Family Play sessions. The completion trigger is also narrowed: it continues to require a completed verified attempt for direct completion, but accepts a classic completion only after the controller and every invited learner have submitted a distinct final lock. Existing family authorization, controller lease, participant presence, quiz lock-in, final challenge lock-in, status transition controls, and individual verified-attempt protections remain unchanged. No answers, progress totals, or private learner data are exposed.
