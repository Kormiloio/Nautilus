...
| FR-45 | An authorized Family Play controller can navigate classic shared-session segments when the crew is ready, and a Family Play session completes when every required participant locks the final challenge; both behaviors must remain compatible with any linked verified attempt. | Must |
...
### Authorized live family pilot and complete-course authoring

The family explicitly requested activation in normal learner lessons, not only adult preview. Release Montenegrin days 24–40 as a visibly labeled family pilot using catalog revision 6. Draft language metadata remains truthful; this pilot authorization is not fluent review. Both individual and Family Play sessions use the daily allocations. Existing sessions retain their pinned older revision, completed progress is untouched, and other packs retain their current content. Author a complete 200-day vocabulary inventory and daily allocation as a separate draft; its publication is independent of this immediate continuation rollout.

The authorized continuation now extends through day 200: days 24–200 introduce 1,770 words, ten per day, with ten review words drawn only from the legacy baseline or earlier continuation days. The original 24–40 allocations remain identical. The complete 1–200 draft is also authored (2,010 lexical entries), but its first 23-day redesign is not substituted beneath the existing family's history.
