...
begin
  ...
  for exercise in select value from jsonb_array_elements(new.exercises) loop
    if exercise->>'kind' is null or exercise->>'kind' not in ('quiz','match','sentence_builder','sentence_completion','self_report') then
    ...
    elsif exercise->>'kind'='sentence_completion' then
      if jsonb_typeof(exercise->'answer') is distinct from 'string'
         or jsonb_typeof(exercise->'choices') is distinct from 'array'
         or jsonb_array_length(exercise->'choices') < 2
         or not (exercise->'choices' @> jsonb_build_array(exercise->'answer'))
         or jsonb_typeof(exercise->'displayTokens') is distinct from 'array'
         or jsonb_typeof(exercise->'completedSentence') is distinct from 'array'
         or jsonb_array_length(exercise->'displayTokens') <> jsonb_array_length(exercise->'completedSentence')
         or coalesce(exercise->>'blankIndex','') !~ '^[0-9]+$'
         or (exercise->>'blankIndex')::integer >= jsonb_array_length(exercise->'displayTokens')
         or exercise->'displayTokens'->((exercise->>'blankIndex')::integer) <> '"____"'::jsonb
         or exercise->'completedSentence'->((exercise->>'blankIndex')::integer) <> exercise->'answer' then
        raise exception 'Sentence completion needs a reviewed blank, choices, and completed sentence';
      end if;
    end if;
  end loop;
  return new;
...
begin
  ...
  select jsonb_agg(case when value->>'kind'='sentence_completion' then value-'answer'-'completedSentence' else value-'answer' end order by ordinality) into public_exercises
  ...
  return jsonb_build_object('id',a.id,'status',a.status,'lessonId',c.lesson_id,
    ...
    'receipts',(select coalesce(jsonb_agg(jsonb_build_object('index',exercise_index,'correct',correct,
      'reward',reward,'answer',answer,'correctAnswer',case when c.exercises->exercise_index->>'kind' in ('quiz','sentence_builder','sentence_completion') then c.exercises->exercise_index->'answer' else null end,'completedSentence',case when c.exercises->exercise_index->>'kind'='sentence_completion' then c.exercises->exercise_index->'completedSentence' else null end,'evidence',evidence_kind) order by exercise_index),'[]'::jsonb)
...
begin
  ...
  if exercise->>'kind'='self_report' then
    ...
    if exercise->>'kind'='sentence_completion' and jsonb_typeof(response) is distinct from 'string' then
      raise exception 'Sentence completion needs one offered word'; end if;
    if exercise->>'kind'='sentence_completion' and not (exercise->'choices' @> jsonb_build_array(response)) then
      raise exception 'Answer is not an offered choice'; end if;
  ...
  return jsonb_build_object('correct',result,'reward',points,'completed',total_received=total_required,
    ...
    'correctAnswer',case when exercise->>'kind' in ('quiz','sentence_builder','sentence_completion') then exercise->'answer' else null end,
    'completedSentence',case when exercise->>'kind'='sentence_completion' then exercise->'completedSentence' else null end);
end;
$$;
